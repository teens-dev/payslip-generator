 ...
 * Color Utilities for html2canvas Compatibility
 *
 * The key insight: window.getComputedStyle() on the LIVE DOM always returns
 * resolved rgb() values — even when Tailwind CSS v4 uses lab()/oklch() internally.
 * So we read from the live element and bake rgb values in as inline styles
 * BEFORE html2canvas clones the element. The clone then inherits safe rgb values.
 */
/**
 * Color Utilities for html2canvas Compatibility
 *
 * ROOT CAUSE: html2canvas parses <style> and <link> stylesheet tags in the
 * cloned document and crashes on lab()/oklch() color functions from Tailwind v4.
 *
 * THE FIX:
 * 1. BEFORE html2canvas runs: bake all computed rgb() values as inline styles
 *    on the LIVE element (browser resolves lab→rgb for us).
 * 2. In onclone: remove ALL <style>/<link> tags from the cloned document so
 *    html2canvas never parses them. Inline styles are all that's needed.
 */
...
/**
 * Step 1: Call on the LIVE element BEFORE html2canvas.
 * Bakes all computed styles as inline styles so the clone has everything it needs.
 */
export function stripUnsupportedColors(element: HTMLElement): void {
...
/**
 * Step 2: onclone callback — remove ALL stylesheets from cloned document.
 * This is the critical step: prevents html2canvas from ever seeing lab()/oklch().
 */
export function getHtml2CanvasOnCloneCallback() {
  return (clonedDoc: Document) => {
    try {
      // Remove all <style> tags — this kills lab()/oklch() at the source
      Array.from(clonedDoc.querySelectorAll('style'))
        .forEach((s) => s.parentNode?.removeChild(s));

      // Remove all <link rel="stylesheet"> tags
      Array.from(clonedDoc.querySelectorAll('link[rel="stylesheet"]'))
        .forEach((l) => l.parentNode?.removeChild(l));
    } catch (e) {
      console.warn('html2canvas onclone error:', e);
    }
  };
}